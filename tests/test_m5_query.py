...
def test_compile_order_by():
    import examples.platform as p
    stmt = select(p.Bug).where(p.Bug.c.severity == "S1").order_by(
        p.Bug.c.created, "DESC",
    )
    jql = stmt.compile(_platform_state())
...
async def test_session_scalars_dc_returns_hydrated_instances():
    ...
    respx.get(f"{DC_ROOT}/search").mock(return_value=httpx.Response(200, json={
        "issues": [
            {"key": "PLAT-1", "fields": {
...
async def test_session_get_by_key_hits_issue_endpoint():
    ...
    respx.get(f"{DC_ROOT}/issue/PLAT-7").mock(return_value=httpx.Response(200, json={
        "key": "PLAT-7", "fields": {
...
async def test_identity_map_returns_same_instance():
    ...
    respx.get(f"{DC_ROOT}/issue/PLAT-1").mock(return_value=httpx.Response(200, json={
        "key": "PLAT-1", "fields": {
...
async def test_scalars_then_get_reuses_identity():
    ...
    respx.get(f"{DC_ROOT}/search").mock(return_value=httpx.Response(200, json={
        "issues": [{"key": "PLAT-1", "fields": {
            "summary": "s", "reporter": {"name": "alice"},
            "customfield_10042": {"value": "S1"},
        }}],
...
async def test_session_scalars_cloud_uses_search_jql():
    ...
    respx.post(f"{CLOUD_ROOT}/search/jql").mock(return_value=httpx.Response(200, json={
        "issues": [
            {"key": "PLAT-1", "fields": {
                "summary": "cloud bug",
...
async def test_cloud_pagination_via_next_page_token():
    ...
    respx.post(f"{CLOUD_ROOT}/search/jql").mock(side_effect=[
        httpx.Response(200, json={
            "issues": [{"key": "PLAT-1", "fields": {
                "summary": "p1", "reporter": {"accountId": "a"},
                "customfield_10042": {"value": "S1"},
            }}],
            "nextPageToken": "tok-2",
        }),
        httpx.Response(200, json={
            "issues": [{"key": "PLAT-2", "fields": {
                "summary": "p2", "reporter": {"accountId": "b"},
...
async def test_dc_pagination_via_start_at():
    ...
    respx.get(f"{DC_ROOT}/search").mock(side_effect=[
        httpx.Response(200, json={
            "issues": [{"key": "PLAT-1", "fields": {
                "summary": "p1", "reporter": {"name": "a"},
                "customfield_10042": {"value": "S1"},
            }}],
