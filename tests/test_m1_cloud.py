...
def _stub_empty(mock: respx.MockRouter) -> None:
    ...
    mock.get(f"{CLOUD_ROOT}/issuetypescreenscheme").mock(
        return_value=httpx.Response(200, json=_paginated([]))
    )
    mock.get(f"{CLOUD_ROOT}/fieldconfiguration").mock(
        return_value=httpx.Response(200, json=_paginated([]))
    )
    mock.get(f"{CLOUD_ROOT}/fieldconfigurationscheme").mock(
        return_value=httpx.Response(200, json=_paginated([]))
    )
...
async def test_cloud_select_options_fetched_via_default_context():
    ...
    respx.get(f"{CLOUD_ROOT}/field").mock(return_value=httpx.Response(200, json=[
        {
...
async def test_cloud_reflects_tmp_project_style():
    ...
    respx.get(f"{CLOUD_ROOT}/project/search").mock(
        return_value=httpx.Response(200, json=_paginated([
            ...
            {
                "id": "10010", "key": "TMP1", "name": "Team One",
